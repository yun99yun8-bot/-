"""V8.1.2 Production Background Worker.
...
print(f'[V8.1.2] worker started instance={INSTANCE_ID} model={core.MODEL_VERSION}', flush=True)
...
while not _stop:
    ...
    if now-last_log >= 30:
        ...
        print(f'[V8.1.2] heartbeat period={ps} db={"ok" if ok else "error"} engines={len(alive)} {alive}', flush=True)
        last_log=now
    time.sleep(5)
...
try:
    ...
    pass
print('[V8.1.2] worker stopping', flush=True)
